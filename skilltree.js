...
function initSkillTreeUI() {
    ...
    if (!container) return;
    container.innerHTML = `<div class="st-wrapper" id="st-wrapper"><svg class="st-svg-layer" id="st-svg-layer"></svg><div id="st-nodes-layer"></div></div><div class="st-info-panel" id="st-info-panel"><div class="st-info-title">HOVER OVER A NODE</div><div class="st-info-desc">Explore the web to see upgrade names, effects, and costs.</div><div class="st-info-cost"></div></div>`;
    ...
    Object.keys(SKILL_TREE).forEach(key => {
        ...
        node.reqs.forEach(reqKey => {
            ...
            if (!parent) return;
            svgLines += `<line id="line-${reqKey}-${key}" class="st-line" x1="${parent.x}%" y1="${parent.y}%" x2="${node.x}%" y2="${node.y}%" />`;
        });
...
function renderSkillTree() {
    ...
    Object.keys(SKILL_TREE).forEach(key => {
        ...
        const reqsMet = node.reqs.length === 0 || node.reqs.every(req => (window.gameData.skillTreeLevels[req] || 0) > 0);
        ...
        node.reqs.forEach(reqKey => {
            ...
            if (!line) return;
            if ((window.gameData.skillTreeLevels[reqKey] || 0) > 0) line.classList.add('active');
...
function updateSkillInfoPanel(key) {
    ...
    const reqsMet = node.reqs.length === 0 || node.reqs.every(req => (window.gameData.skillTreeLevels[req] || 0) > 0);
    let costText;
    if (currentLevel >= node.maxLevel) costText = '<span style="color:#00ff9d;">[MAXED]</span>';
    else if (!reqsMet) {
        const need = node.reqs.filter(r => !(window.gameData.skillTreeLevels[r] || 0)).map(r => SKILL_TREE[r] ? SKILL_TREE[r].name : r).join(', ');
        costText = `<span style="color:#ff6666;">Requires: ${need}</span>`;
    } else {
        costText = `Cost: ${node.cost} RP` + (window.gameData.rebirthPoints < node.cost ? ' <span style="color:#ff6666;">(not enough RP)</span>' : '');
    }
    const status = currentLevel > 0 ? ` (Level ${currentLevel}/${node.maxLevel})` : ' (Locked)';
    panel.innerHTML = `<div class="st-info-title">${node.name}${status}</div><div class="st-info-desc">${node.desc}</div><div class="st-info-cost">${costText}</div>`;
}

function resetSkillInfoPanel() {
    const panel = document.getElementById('st-info-panel');
    if (!panel) return;
    panel.innerHTML = `<div class="st-info-title">HOVER OVER A NODE</div><div class="st-info-desc">Explore the web to see upgrade names, effects, and costs.</div><div class="st-info-cost"></div>`;
}

function loadSkillTree() {
    if (!window.gameData.skillTreeLevels) window.gameData.skillTreeLevels = {};
}
